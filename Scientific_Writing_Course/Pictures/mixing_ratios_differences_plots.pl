...
$R->run(q` mech.colours = c("MCM v3.2" = "#6c254f", "MOZART-4" = "#ef6638", "RADM2" = "#0e5c28") `);
foreach my $run (sort keys %data) {
    ...
    $R->set('mech.filename', "${run}_${species}_mixing_ratios_facet_mechanism.pdf");
    $R->set('spec.filename', "${run}_${species}_mixing_ratios_facet_speciation.pdf");
    ...
    $R->run(q` plot = ggplot(data, aes(x = Time, y = Mixing.Ratio, colour = Speciation, group = Speciation)) `,
            ...
            q` plot = plot + theme(strip.text = element_text(face = "bold", size = 12)) `,
            #q` plot = plot + theme(plot.title = element_text(face = "bold")) `,
            q` plot = plot + theme(axis.title = element_text(face = "bold", size = 12)) `,
            q` plot = plot + theme(panel.margin = unit(4, "mm")) `,
            q` plot = plot + theme(legend.title = element_blank()) `,
            q` plot = plot + theme(legend.text = element_text(size = 11)) `,
    ...
    $R->run(q` Cairo(file = mech.filename, type = "pdf", bg = "transparent", units = "cm", width = 27, height = 10.3) `,
            q` print(plot) `,
            q` dev.off() `,
    );
    ...
    $R->run(q` plot = ggplot(data, aes(x = Time, y = Mixing.Ratio, colour = mechanism, group = mechanism)) `,
            ...
            q` plot = plot + theme(legend.position = c(1.03, 1.03)) `,
            q` plot = plot + theme(legend.justification = c(1, 1)) `,
            q` plot = plot + scale_colour_manual(values = mech.colours) `,
            q` plot = plot + theme(legend.position = "top") `,
    );

    $R->run(q` Cairo(file = spec.filename, type = "pdf", bg = "transparent", units = "cm", width = 27, height = 10.3) `,
            q` print(plot) `,
            q` dev.off() `,
    );
};
